            ...
            <div className="mt-10 overflow-hidden border-y border-mercury-200" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
                <div ref={trackRef} className="flex w-max">
                    {[...props.slides, ...props.slides].map((slide, index) => {
                        const slideClassName = 'group relative flex h-24 w-48 shrink-0 items-center justify-center overflow-hidden border-r border-mercury-200 bg-mercury-50 px-8';
                        const slideContent = (
                            <>
                        ...
                        return slide.link ? (
                            <AppLink key={index} href={slide.link.href} title={slide.link.title} className={slideClassName}>
                                {slideContent}
                            </AppLink>
                        ) : (
