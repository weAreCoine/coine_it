...
class Iubenda
{
    ...
    // scripts
    public $auto_script_tags = [];

    // iframes
    public $auto_iframe_tags = [];
    ...
    // per-purpose iframes
    public $iub_comments_detected = [];

    public $skipped_comments_detected = [];

    public $iframes_skipped = [];

    public $iframes_detected = [];

    public $iframes_converted = [];

    public $scripts_el = [];

    public $scripts_skipped = [];

    public $scripts_detected = [];

    public $scripts_converted = [];

    public $scripts_inline_skipped = [];

    public $scripts_inline_detected = [];

    public $scripts_inline_converted = [];
     ...
     * @param  array  $args
     */
    public function __construct($content_page = '', $args = [])
    {
        // valid type?
        $this->type = ! empty($args['type']) && in_array($args['type'], ['page', 'faster'], true) ? $args['type'] : 'page';
    ...
    public static function get_purposes()
    {
        ...
        if (! empty($_COOKIE)) {
            foreach ($_COOKIE as $key => $value) {
                $found = self::strpos_array($key, ['_iub_cs-s', '_iub_cs']);
    ...
    public function parse_comments()
    {
        ...
        // block
        foreach (['IUB_REGEX_PATTERN', 'IUB_REGEX_PATTERN_2', 'IUB_REGEX_PURPOSE_PATTERN'] as $pattern) {
    ...
    {
        return [$this->iub_class, $this->iub_class_inline];
    }
}
